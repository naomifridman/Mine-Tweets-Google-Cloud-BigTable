...
if __name__ == '__main__':
  ...
  for i in range(n_times):
    ...
    print('query', query, 'Qey_words',Qey_words)
    ...
    if (i == 0):
           ...
           print('df_results after concar:', df_results.head())
...
if __name__ == '__main__':
  ...
  for i in range(n_times):
    ...
    # Add results to df_results
    if (i == 0):
           df_results = pd.DataFrame({re.sub(r'\W+', '', str(args.locations[0])) + '_words':word_list,
                              re.sub(r'\W+', '',str(args.locations[0])) + '_freq':freq_list})
           print('df create',df_results.head())
    else:
           df1 = pd.DataFrame({re.sub(r'\W+', '', str(args.locations[0])) + '_words' : word_list,
                              re.sub(r'\W+', '',str(args.locations[0])) + '_freq':freq_list})
           print('df1',df1.head())
           print('df_results', df_results)
           df_results = pd.concat([df_results,df1], ignore_index=True, axis=1)
